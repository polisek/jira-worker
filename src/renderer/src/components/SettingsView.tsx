...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="settings-card mb-4">
      <h2 className="text-sm font-semibold text-gray-300 mb-4 pb-2 border-b border-gray-700/60">{title}</h2>
...
export function SettingsView({ onSaveJira, onSavePrefs, initialJira, prefs }: Props) {
  ...
  return (
    <div className="w-full max-w-xl py-6 px-2">
      <div className="mb-6">
        <h1 className="text-xl font-bold text-gray-100 mb-1">Nastavení</h1>
        <p className="text-sm text-gray-500">Přizpůsobte si chování aplikace</p>
      </div>
      ...
      <Section title="Jira účet">
        <div>
          ...
          <input
            type="url"
            value={baseUrl}
            onChange={(e) => { setBaseUrl(e.target.value); setJiraSaved(false) }}
        ...
        <div>
          ...
          <input
            type="email"
            value={email}
            onChange={(e) => { setEmail(e.target.value); setJiraSaved(false) }}
        ...
        <div>
          ...
          <div className="relative">
            <input
              ...
              value={apiToken}
              onChange={(e) => { setApiToken(e.target.value); setJiraSaved(false) }}
          ...
          <p className="text-xs text-gray-600 mt-1">Token se ukládá pouze lokálně</p>
        </div>
