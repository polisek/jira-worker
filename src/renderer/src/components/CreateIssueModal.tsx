        ...
        <div className="flex-1 overflow-y-auto p-5 flex flex-col gap-4">
          ...
          <Field label="Štítky">
            <div className="flex flex-wrap gap-1.5 mb-2">
              {labels.map((l) => (
                <span key={l} className="badge badge-gray flex items-center gap-1">
                  {l}
                  <button onClick={() => setLabels((prev) => prev.filter((x) => x !== l))} className="text-gray-500 hover:text-gray-300">×</button>
                </span>
              ))}
            </div>
            <div className="flex gap-2">
              ...
              />
              <button onClick={addLabel} className="btn-secondary px-3 text-sm">Přidat</button>
            </div>
          </Field>
